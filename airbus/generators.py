...
from airbus.pipelines import validation_classification_pipeline
...
def get_validation_generator(num_folds, fold_ids, batch_size, limit=None, classification=False):
    ...
    if classification:
        transform = partial(validation_classification_pipeline, mask_db)
    else:
...
def get_train_generator(num_folds, fold_ids, batch_size, limit=None, classification=False):
    mask_db = get_mask_db('data/train_ship_segmentations_v2.csv')
    all_image_ids, all_fold_ids = get_fold_split(mask_db, num_folds)
    image_ids = all_image_ids[np.isin(all_fold_ids, fold_ids)]
    image_paths = list(map(lambda id: f'data/train/{id}', image_ids))
    ...
    if classification:
        transform = partial(train_classification_pipeline, mask_db)
    else:
